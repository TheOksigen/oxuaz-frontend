...
export default function Header() {
  ...
  return (
    <header className="bg-white dark:bg-indigo-900">
      <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
        <div className="flex lg:flex-1">
          <Link to="/" className="-m-1.5 p-1.5">
            ...
            <img
              alt=""
              src="https://oxu.az/media/img/logo.svg?v=1"
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {isLoading ? <Spinner /> : categories?.map((item) => (
            <Link key={item.name} to={`/categories/${item._id}`} className="border border-sm px-4 rounded-xl text-sm/6 font-semibold text-black">
              {item.name}
            </Link>
          ))}
        </div>
